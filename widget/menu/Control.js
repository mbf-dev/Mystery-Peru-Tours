...
export default class Control extends React.Component {
  ...
  loadOptions = (inputValue, callback) => {
    ...
    alert(JSON.stringify(inputValue, null, 4));
  ...
  handleSelectChangedFor = parent => {
    return (newValue, event) =>
      this.handleSelectChange(newValue, event, parent);
  };
  ...
  deleteRootItem = item => {
    const { onChange } = this.props;
    const index = this.getValue().findIndex(
      x => item.title === x.get(FIELD_TITLE)
    );
    onChange(this.getValue().delete(index));
  };
  ...
  moveRootItem = (item, up) => {
    ...
    const newList = this.getValue()
      .delete(index)
      .insert(index + offset, listItem);
    onChange(newList);
  };
